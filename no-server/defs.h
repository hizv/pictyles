...
struct PicParams {
  ...
  uint32_t sim_box_length, box_count, lb_freq, migrate_freq;
  PicParams() {}
  PicParams(uint8_t pos_distribution_, uint8_t geometry_, uint32_t odf_,
            ...
            float time_delta_, uint32_t lb_freq_, uint32_t migrate_freq_)
      : pos_distribution(pos_distribution_), geometry(geometry_), odf(odf_),
        initial_particle_count(initial_particle_count_),
        max_iterations(max_iterations_), time_delta(time_delta_), lb_freq(lb_freq_), migrate_freq(migrate_freq_) {
    alpha = 0.0;
    beta = 0.0;
  }
};
